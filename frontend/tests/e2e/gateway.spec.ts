...
test.describe('Authentication', () => {
  ...
  test('should show error for invalid credentials and stay on login', async ({ page }) => {
    await page.goto('/');
...
test.describe('API List', () => {
  test('should toggle between API List and Store Account tabs', async ({ page }) => {
    ...
    await expect(page.locator('div.table-cell').filter({ hasText: 'API Name' })).toBeVisible();

    await page.click('button:has-text("Store Account")');
    await expect(page.locator('div.table-cell').filter({ hasText: 'Store Name' })).toBeVisible();
  ...
  test('should add, edit, and delete an API', async ({ page }) => {
    ...
    await page.fill('.modal-box input[placeholder="e.g. My-Awesome-API"]', apiName);
    await page.fill('.modal-box input[placeholder="e.g. my-awesome-api"]', apiPath);
    ...
    await expect(page.locator('input[placeholder="https://api.example.com"]')).toHaveValue('https://api.thecatapi.com');

    const renamed = `${apiName}-renamed`;
    page.on('dialog', (dialog) => dialog.accept());
    await page.locator('input[placeholder="API name"]').fill(renamed);
    await page.click('button:has-text("Save Changes")');
    await expect(page.locator('div.toast-success')).toContainText('saved successfully');
  ...
  test('should show detail page for seeded API', async ({ page }) => {
    ...
    await expect(page.locator('input[placeholder="https://api.example.com"]')).toHaveValue('https://api.thecatapi.com');
    await expect(page.locator('input[placeholder="/v1/endpoint"]')).toHaveValue('/v1/images/search');
  ...
  test('should execute API via Try It modal', async ({ page }) => {
    ...
    await expect(page.locator('.modal-box pre')).toContainText(/cdn2\.thecatapi\.com|Request failed|❌/);
  });
});

test.describe('Store Account', () => {
  test('should add, edit, regenerate key, and delete a store', async ({ page }) => {
    page.on('dialog', (dialog) => dialog.accept());
    await login(page);
    await page.click('button:has-text("Store Account")');
    ...
    await expect(page.getByRole('button', { name: '+ Regenerate Key' })).toBeEnabled({ timeout: 20000 });
    await expect(page.locator('div.toast-success')).toContainText('New API key generated!');
